...
SELECT gender, AVG(height) FROM students GROUP BY gender; -- 성별을 기준으로 그룹화하겠다

-- 그룹화의 특징 3가지
-- 1) 집계 함수와 함께 사용해야 함
-- 그룹별로 유의미한 분석을 얻기 위해서 집계 함수와 함께 사용
-- 단순 GROUP BY절만 사용하는 것은 데이터를 그룹으로 묶기만 함(의미 없음)
...
SELECT city AS 도시, YEAR(sale_date) AS '판매 연도', SUM(amount) AS '총 매출' -- 3) 특정 도시의 연도별 통 매출 집계 조회 
...
-- 잘못된 컬럼 사용 예시 
SELECT id, gender, AVG(height) FROM students GROUP BY gender;
...
FROM 테이블명
WHERE 일반_필터링_조건 -- 그룹화하기 전에 개별 행(row)에 대해 필터링하는 행 단위 필터링(집계 함수 사용 불가)
GROUP BY 그룹화_컬럼
HAVING 그룹_필터링_조건; -- 그룹핑 결과에 대해 필터링하는 그룹 단위 필터링(집계 함수 사용 가능)

-- 결제 유형별 평균 결제 금액이 40,000원 이상인 데이터는?
-- 우선 결제 유형별(GROUP BY) 평균 결제 금액 구하기
SELECT ptype AS '결제 유형', AVG(amount) AS '평균 결제 금액' 
FROM payments GROUP BY ptype;

-- 위 결과에 40,000원 이상인 데이터 구하기 => HAVING
SELECT ptype AS '결제 유형', AVG(amount) AS '평균 결제 금액' 
FROM payments GROUP BY ptype HAVING AVG(amount) >= 40000; -- 집계 함수는 HAVING절에서 조건을 거는 게 맞음
-- WHERE AVG(amount) >= 40000
-- 오류 발생 : WHERE절은 그룹화가 이루어지기 전, 테이블의 개별 행 하나하나에 대해 조건을 검사하기 때문
...
SELECT ptype AS '결제 유형', AVG(amount) AS '평균 결제 금액' 
FROM payments GROUP BY ptype HAVING `평균 결제 금액` >= 40000; -- 주의! ''로 묶으면 문자 데이터로 인식
...
-- ORDER BY 절을 사용하여 수행
-- SELECT로 조회한 데이터를 기준으로 정렬하는 작업(SELECT가 먼저 수행됨)
...
SELECT u.nickname AS '주문자명', SUM(amount) AS '주문 총액' -- 6) 특정 컬럼을 조회 
...
SELECT p.name AS '상품명', SUM(count) AS '판매수량' -- 6) 
FROM products p -- 1)
JOIN order_details od ON p.id = od.product_id -- 2) orders 테이블에 장바구니 정보도 함께 저장되어서 이를 구분하기 위해 JOIN을 한번 더하고
JOIN orders o ON o.id = od.order_id -- 3)
WHERE status = '배송 완료' -- 4) 배송 완료 레코드만 필터링
