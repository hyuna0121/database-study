...
-- 삭제 전에는 항상 백업이 되어 있는 지, 그리고 정말 삭제해야 하는 대상이 맞는 지 확인하는 습관
...
-- 기본 키 : 레코드(row)를 대표하는 컬럼(예 : 사람의 주민등록번호, 학생의 학번, 군인의 군번, 사원의 사번)
...
UPDATE burgers
SET price = 500
WHERE name = '빅맨'; -- 특정 대상 변경 시, 조건은 반드시 기본 키를 사용
-- 안전모드에서는 KEY 컬럼이 아닌 다른 값을 사용하면 막음

-- 수정 대상 조건 개성 : 키를 통한 변경 쿼리
UPDATE burgers
SET price = 500
WHERE id = 1;
...
DELETE FROM burgers
WHERE name = '슈비두밥 버거'; -- 에러 발생 : 수정과 동일하게 조건은 반드시 기본 키를 사용
