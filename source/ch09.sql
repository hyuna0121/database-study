...
*/
-- 왜 필요할까?
-- JOIN만으로는 한 번에 답하기 어려운, 여러 단계의 질의를 거쳐야 하는 문제들도 있음
...
FROM order_details GROUP BY order_id; -- 서브쿼리로 사용

-- 메인쿼리 : 1회 주문 시 평균 상품 개수 집계
SELECT AVG(sub.total_count) AS '1회 주문 시 평균 상품 개수'
FROM (
    -- 서브쿼리
    SELECT order_id, SUM(count) AS total_count -- 집계 함수 결과에 별칭 필수(칼람명이 아니라 계산된 값을 반환하기 때문에)
...
FROM order_details GROUP BY product_id; -- 서브쿼리로 사용

-- 메인쿼리 : 상품명을 포함한 상품별 주문 개수 집계
SELECT p.name AS '상품명', sub.total_count AS '주문개수'
FROM products p 
JOIN (
    SELECT product_id, SUM(count) AS total_count
    FROM order_details 
    GROUP BY product_id) AS sub ON p.id = sub.product_id;
...
-- 메인쿼리 : 전체 상품의 매출을 조회
SELECT p.name AS '상품명', SUM(count * price) AS '매출'
...
-- 메인 쿼리 : 우유 식빵이나 플레인 베이글보다 저렴한 상품이 있는 지 조회
SELECT * FROM products
WHERE price < ANY (
    -- 서브쿼리 : 우유 식빵과 플레인 베이글의 가격 조회
    SELECT price FROM products 
    WHERE name IN ('우유 식빵', '플레인 베이글') -- 2900, 1300
); -- 최대 값보다 작으면 참, 결과적으로 2900원보다 작은 모든 상품 조회
...
-- 메인 쿼리 : 우유 식빵과 플레인 베이글 2가지보다 가격이 더 높은 상품 조회
SELECT * FROM products
WHERE price > ALL (
    -- 서브쿼리 : 우유 식빵과 플레인 베이글의 가격 조회
    SELECT price FROM products 
    WHERE name IN ('우유 식빵', '플레인 베이글') -- 2900, 1300
); -- 최대 값보다 커야 참, 결과적으로 2900원보다 큰 모든 상품 조회
...
-- 결과행 1개만 발견되는 바로 TRUE (서브쿼리 중단)
-- 결과 값에는 관심이 없고 반환되는 행이 있는지만 봄
...
WHERE EXISTS (
    ...
    SELECT 1 -- 굳이 모든 컬럼을 다 가져오는 것보다 1만 써주는 게 효율적이고 관례 
    FROM orders o 
    WHERE o.user_id = u.id -- 이렇게 서브쿼리가 메인쿼리의 특정 값을 참조하는 쿼리를 '상관 서브쿼리'라고 함
);
-- (참고)
-- users 테이블 1번 사용자의 주문이 있는 지
...
SELECT * FROM users WHERE id IN (
    SELECT DISTINCT user_id FROM orders -- 서브쿼리의 결과 값이 많아질수록 성능 저하
);
