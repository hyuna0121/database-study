...
-- 테이블 조인 형식
SELECT 컬럼명1, 컬렴명2, ...
FROM 테이블A -- 기준이 되는 첫 번째 테이블 지정
JOIN 테이블B -- 연결할 두 번째 테이블 지정(JOIN만 명시하면 기본적으로 INNER JOIN으로 해석)
ON 테이블A.조인_컬럼 = 테이블B.조인_컬럼; -- (중요) 두 테이블을 어떤 조건으로 연결할 지 명시하는 연결고리
...
-- ON : 댓글 테이블의 외래 키 = 사용자 테이블의 기본 키가 같은 것끼리
...
SELECT * FROM users 
JOIN comments ON users.id = comments.user_id; -- 조인 컬럼(기본키) = 조인 컬럼(외래키)
...
SELECT comments.id AS comment_id, body, users.id AS user_id, nickname
FROM comments
...
SELECT c.body, u.nickname
FROM comments AS c
JOIN users AS u ON c.user_id = u.id;
-- 실무에서 테이블에 별칭을 붙일 때는 AS를 보통 생략
-- (참고) SELECT 절에서 컬럼에 붙이는 별팅은 AS를 생략 안함
...
SELECT * FROM photos p LEFT JOIN users u ON p.user_id = u.id 
UNION -- 두 쿼리의 결과 테이블을 위아래로 합치고 중복 데이터는 제거
SELECT * FROM photos p RIGHT JOIN users u ON p.user_id = u.id;
...
SELECT nickname AS 게시자, filename AS 파일명 FROM photos p 
JOIN users u ON p.user_id = u.id AND nickname = '홍팍';
...
SELECT body AS '댓글 내용', filename AS 파일명 FROM comments c 
