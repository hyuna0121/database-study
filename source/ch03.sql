...
-- Quiz : 가격이 5500원보다 비싼 버거 찾기
SELECT * FROM burgers WHERE price > 5500;

-- Quiz : 가격이 5500원보다 싼 버거 찾기
SELECT * FROM burgers WHERE price < 5500;
...
INSERT INTO burgers (id, name, price, gram, kcal, protein)
VALUES (6, NULL, 8000, 222, 333, 44);

-- SELECT * FROM burgers WHERE name = NULL;
...
-- Quiz : 5500원보다 싸고, 동시에 단백질량이 25g보다 많은 버거
SELECT * FROM burgers WHERE price < 5500 AND protein > 25;

-- Quiz : 5500원보다 싸거나, 단백질량이 25g보다 많은 버거
SELECT * FROM burgers WHERE price < 5500 OR protein > 25;

-- Quiz : 단백질량이 25g보다 많지 않은 버거
SELECT * FROM burgers WHERE NOT protein > 25;
-- SELECT * FROM burgers WHERE !(protein > 25); -- (참고) &&, ||, !는 MYSQL에서만 작동
...
-- 3. 모든 학생의 총점은?
SELECT nickname, math + english + programming AS total FROM students;

-- 4. 모든 학생의 평균은?
SELECT nickname, (math + english + programming)/ 3 AS avg FROM students;

-- 5. 총점이 270 이상인 학생의 닉네임, 총점, 평균은?
SELECT nickname, math + english + programming AS total, (math + english + programming)/ 3 AS avg  
FROM students
