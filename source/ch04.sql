...
-- (참고)
SELECT *, MAX(price), MIN(price) FROM burgers;
-- 이 둘은 동시에 쓸 수 없음, 집계는 그룹 단위 연산이고 *는 행 단위 열람이기 때문
-- "어떤 행의 값을 보여줄 지?" MYSQL이 결정할 수 없어서 에러를 뱉음
...
SELECT *, MAX(price), MIN(price) FROM burgers GROUP BY category;
-- category별로 그룹을 묶고, 각 그룹에서 가격의 최대 값/최소 값을 구함
...
-- 거래 금액의 최대 값/최소 값 구하기
SELECT MAX(amount) AS '최대 거래 금액', MIN(amount) AS '최소 거래 금액' FROM transactions;

-- 페이팔과 거래한 금액의 최대 값/최소 값은?
SELECT MAX(amount) AS '최대 거래 금액', MIN(amount) AS '최소 거래 금액' FROM transactions WHERE msg = 'PayPal';
...
SELECT DISTINCT COUNT(msg) FROM transactions; -- msg의 개수 27에 대한 중복 제거
