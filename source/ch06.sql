...
-- 1부터 시작하여 데이터가 추가될 때마다 1씩 자동으로 증가하는 정수 값(INTEGER)을 기본키로 사용하는 경우가 가장 흔함


-- 외래키(FK, Foreign Key)
-- 다른 테이블의 기본키를 가리키는 컬럼
-- 두 테이블을 관계로 묶어 연결하는 역할
-- 예 : 고객 테이블(PK : customer_id, 참조되는 쪽, 부모테이블) <--(참조)-- 주문 테이블(FK : customer_id, 참조하는 쪽, 자식테이블)
...
CREATE Table users (
    ...
    status VARCHAR(10) DEFAULT 'active', -- 계좌 상태(기본 값은 'active')
...
VALUES ('test@gmail.com', NULL);
-- Error Code : 1048. Column 'name' cannot be null

-- 이메일(UNIQUE)에 중복 값을 입력 시도
INSERT INTO users (email, name)
VALUES ('test@gmail.com', '김');

INSERT INTO users (email, name)
VALUES ('test@gmail.com', '이');
-- Error Code: 1062. Duplicate entry 'test@gmail.com' for key 'users.email'

-- 기본키(PK)에 중복 값을 입력 시도
INSERT INTO users (id, email, name)
VALUES (1, 'test2@gmail.com', '이');
-- Error Code: 1062. Duplicate entry '1' for key 'users.PRIMARY'

-- 나이(age >= 18)에 18세 미만 값을 입력 시도
INSERT INTO users (id, email, name)
VALUES ('test3@gmail.com', '이', 15);
...
-- (참고) Tip! 날짜와 기본 값 설정 옵션
CREATE Table boards (
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
);
-- 이 기능을 사용하면 행이 추가되거나 변경된될 때 날짜를 편리하게 관리할 수 있음
-- DEFAULT CURRENT_TIMESTAMP : 새로운 데이터 행이 추가될 때, 해당 컬럼에 별도의 값을 지정하지 않으면 현재 날짜와 시간이 자동으로 입력됨
-- DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP : 새로운 데이터 행이 추가될 때는 물론이고, 같은 행의 컬럼 값이 변경되어 업데이트될 깨, 현재 날짜와 시간으로 자동 갱신됨
...
DELETE FROM countries WHERE id = 1; -- 이 나라를 참조하는 수도(자식 테이블)가 있기 때문에 삭제 불가
...
CREATE Table players(
    ...
    -- teams 테이블의 id 컬럼을 가르킴 => 선수가 어느 팀에 소속되어 있는 지를 연결(관계 형성)
);
