...
SELECT name, price, CONCAT(name, ' / ', price, '원') AS name_and_price FROM orders;

-- CONCAT_WS(구분자, 문자열1, 문자열2, ...) : CONCAT과 비슷하지만, 첫 번째 인자로 '구분자를 받아 각 문자열 사이에 자동으로 넣어줌
-- (참고) MYSQL 전용 함수, WS는 'With Separator'의 약자
SELECT name, price, CONCAT_WS(' - ', name, price, quantity) AS order_details FROM orders;
...
SELECT name, CHAR_LENGTH(name) AS char_length, LENGTH(name) AS byte_length FROM orders;
...
-- 4. 특정 범위 필터링하기
-- 1) BETWEEN 연산자
-- 두 값 사이에 속하는 지 확인할 때 사용하는 연산자(두 값을 포함하여 찾음)
