...
FROM members m JOIN member_profiles mp ON m.id = mp.member_id;   
...
-- 정답:
SELECT name AS '이름', date_of_birth AS '생년월일', address AS '주소' 
FROM members m LEFT JOIN member_profiles mp ON m.id = mp.member_id;
...
FROM members m JOIN borrow_records br ON m.id = br.member_id
