...
-- 정답:
SELECT e.name AS '이름', (SELECT COUNT(*) FROM employee_projects WHERE employee_id = e.id) AS '참여 중인 프로젝트 수'
FROM employees e;
...
-- 정답: 
SELECT department_id, name AS '이름' FROM employees
...
-- 정답: 
SELECT sub.name AS '부서 이름', COUNT(*) AS '직원 수'
FROM (
	SELECT d.name
...
-- 정답: 
SELECT e.name, sub.amount
FROM employees e 
JOIN (SELECT employee_id, amount FROM salary_records) AS sub ON e.id = sub.employee_id
ORDER BY amount 
DESC LIMIT 1;
...
-- 정답:
SELECT d.name, AVG(sr.amount) FROM employees e
JOIN departments d ON e.department_id = d.id
JOIN salary_records sr ON e.id = sr.employee_id
GROUP BY d.name
HAVING AVG(sr.amount) > (SELECT AVG(amount) FROM salary_records); 
