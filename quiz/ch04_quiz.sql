...
-- 정답:
SELECT COUNT(*) AS expensive_orders FROM orders WHERE price >= 1000000;
...
-- 정답:
SELECT SUM(price * quantity) AS total_price FROM orders WHERE quantity >= 2 AND price <= 1000000;
...
-- 정답:
SELECT COUNT(DISTINCT region) AS unique_regions FROM orders;
...
-- 정답:
SELECT SUM(quantity) AS total_quantity FROM orders WHERE order_date BETWEEN '2023-11-01' AND '2023-11-05';
...
-- 정답:
SELECT DISTINCT product AS unique_products FROM orders WHERE customer_name = '김철수' OR region = '대구';
